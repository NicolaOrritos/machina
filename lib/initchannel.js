    ...
    start: function(options)
    {
        ...
        var handler = function (req, res)
        {
                ...
                req.method.toLowerCase() === 'post')
            {
                ...
                {
                    self.agents.sub(metadata.agent, metadata.actions);
                    
                    var agentPort = self.agents.port(metadata.agent);
                    
                    answer(res, commands.AACK_WITHPORT(agentPort));
                    
                }).on(commands.AUNSUB, function(metadata)
                {
                    self.agents.unsub(metadata.agent, metadata.actions);
                ...
                }).on(commands.TBEGIN, function(metadata)
                {
                    ...
                    controlchannel.on('message', function(data)
                    {
                        if (data.event === 'ready')
                        {
                            ...
                            // 3. If there is no action specified broadcast everyone;
                            //    notify only interested parties otherwise
                            if (metadata.actions.length)
                            {
                                // Notify agents waiting for actions X,Y,Z to listen for the incoming transaction
                                var interested = self.agents.agents(metadata.actions);
                                self.agents.notify(interested, transaction.portD2, metadata.actions);
                            }
                            else
                            {
                                self.agents.notifyAll(transaction.portD2, metadata.actions);
                            }
