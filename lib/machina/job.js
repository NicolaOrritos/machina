...
function _closeFileDataPool(filePath, finished)
{
    var error;
    ...
    if (filePath)
    {
        ...
        fs.open(outFilePath, 'r', function(err, fd)
        {
            if (err)
            {
                console.log('Could not find "%s". Continuing...', outFilePath);
            }
            else
            {
                fs.rename(outFilePath, filePath, function(err)
                {
                    error = err;
            ...
            // Delete any ".in" file anyway:
            fs.unlink(inFilePath, function(err3)
            {
                console.log('Could not delete "%s" file. Continuing...', inFilePath);
            });
        });
    }
    else if (finished)
...
function _createChildProcess(job, childStdio, isLastOne)
{
    function done(error)
    {
        if (   isLastOne
            && job.conf.DATA_POOL
            && job.conf.DATA_POOL.ENABLED
            && job.conf.DATA_POOL.TYPE === 'FILE')
        {
            _closeFileDataPool(job.conf.DATA_POOL.FILE, function(err2)
            {
                if (err2)
                {
                    console.log('Could not close file data-pool. Cause: %s', err2);
                }
    ...
    if (job)
    {
        ...
        // Under certain conditions there may not be a stderr stream:
        if (child.stderr)
        {
...
Job.prototype.start = function(isLastOne)
{
    var self = this;
        ...
        && this.conf.DATA_POOL.TYPE === 'FILE')
    {
        _prepareFileDataPool(this.conf.DATA_POOL.FILE, function(error, childStdio)
        {
            ...
            {
                _createChildProcess(self, childStdio, isLastOne);
            }
        });
    }
    else
    {
        _createChildProcess(this, 'ignore', isLastOne);
    }
