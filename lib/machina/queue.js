...
Queue.prototype.start = function()
{
    if (this.stopped)
    {
            ...
            if (this.job.start)
            {
                ...
                this.job.start(function(err)
                {
                    if (err)
                    {
                        ...
                        self.emit(Queue.ERROR_EVENT, err, self.job.name);
                    ...
                    else
                    {
                        ...
                        self.emit(Queue.STOPPED_EVENT, at);
                    }
                });
            }
            else
        ...
        else if (this.conf.CYCLE)
        {
            ...
            this.emit(Queue.CYCLE_EVENT);
        }
        else
        {
            this.current = 0;

            this.emit(Queue.FINISHED_EVENT);
        }
    }
    else
    {
...
Queue.prototype.jobsCount = function()
{
