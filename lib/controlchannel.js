...
{
    console.log('[CONTROL_CHANNEL] Exiting from transaction: %s', JSON.stringify(transaction));
...
{
    console.log('[CONTROL_CHANNEL] Exiting for transaction: %s', JSON.stringify(transaction));
    
    process.send({event: 'dead', status: 'SIGKILL', transaction: transaction});
});
